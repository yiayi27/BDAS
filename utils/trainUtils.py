...
from loss.loss import ContrastiveLoss, MSELoss,TotalLoss

class Utils:
    def __init__(self, cfgs):
        self.BCVA_loss = MSELoss()
        self.Consistent_loss = TotalLoss()
    ...
    def train(self, model, train_loader, val_loader):
        ...
        for epoch in range(1, self.cfgs['train_cfg']['Epochs'] + 1):
            ...
            for batch in tqdm(train_loader):
                OctImage, SloImage, label, patientMessage, diagOct, diagSlo, missModalTag = self.unpackToGpu(batch)
                feature1,feature2,predBCVA1,predBCVA2,BCVA, predOct, predSlo, octEmbed, sloEmbed = model.forward(OctImage, patientMessage, SloImage, missModalTag, diagOct, diagSlo)
                optimizer.zero_grad()
                postProcess = []
                for key in label:
                ...
                consLoss=self.Consistent_loss(feature1,feature2,predBCVA1,predBCVA2)
                #在这个位置增加函数，使得我们能求对于这一个batch中的octimg-diagoct以及sloimg-diagslo各个之间的相似度矩阵
                ...
                trainLoss = predBCVALoss + predOctLoss * self.cfgs['train_cfg']['incomplete']['alpha'] + \
                            predSloLoss * self.cfgs['train_cfg']['incomplete']['beta'] + ctrLoss+consLoss
                trainLoss.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                for batch in tqdm(val_loader):
                    ...
                    diagOct, diagSlo, missModalTag = self.unpackToGpu(batch)
                    feature1,feature2,predBCVA1,predBCVA2,BCVA, predOct, predSlo, octEmbed, sloEmbed = model.forward(OctImage,  patientMessage, SloImage, missModalTag, diagOct, diagSlo)
                    postProcess = []
                    for key in label:
                        postProcess.append(self.getValue(key)[0])
    ...
    def test(self, model, test_loader):
        ...
        model.load_state_dict(torch.load(self.model_save_path + '/Best.pt',weights_only=True))
        model = model.cuda()
        model.eval()
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader):
                ...
                diagSlo, missModalTag= self.unpackToGpu(batch)

                feature1, feature2, predBCVA1, predBCVA2, BCVA, predOct, predSlo, octEmbed, sloEmbed = model.forward(
                    OctImage, patientMessage, SloImage, missModalTag, diagOct, diagSlo)
                postProcess = []
                for key in label:
                    postProcess.append(self.getValue(key)[0])
                postProcess = torch.Tensor(postProcess).cuda()
    ...
    def unpackToGpu(self, batch):
        OctImage, SloImage, label, patientMessage, \
        diagOct, diagSlo, missModalTag = batch
        ...
        missModalTag = [x.cuda() for x in missModalTag]


        return OctImage, SloImage, label, patientMessage, \
        diagOct, diagSlo, missModalTag
